...
type CategoryMatrix = {
  columns: string[]; // e.g., "0-1 year", "1-5 years", etc. (more columns)
  rows: string[]; // e.g., "Female", "Male" (fewer rows)
};
...
interface HorizontalMatrixTextFieldBlockProps {
  q: DataElement;
...
const SESSION_STORAGE_KEY_PREFIX = 'horizontal-matrix-field-';

const HorizontalMatrixTextFieldBlock: FC<HorizontalMatrixTextFieldBlockProps> = ({
  q,
  coc,
  period,
  source,
  ...
  const [matrix, setMatrix] = useState<CategoryMatrix>({ columns: [], rows: [] });
  ...
  // Note how column/row logic is flipped compared to vertical version
  useEffect(() => {
    ...
    coc.forEach(c => {
      ...
      if (parts.length >= 2) {
        // Rows are gender (parts[0]), columns are age categories (parts[1])
        // This is the key difference - we're flipping the orientation
        rowValues.add(parts[0]); // e.g., "Female" - now as rows (fewer)
        colValues.add(parts[1]); // e.g., "0-1 year" - now as columns (more)
      }
    });
    ...
    // Initialize collapsed state for each column
    const initialCollapsedState: Record<string, boolean> = {};
    Array.from(colValues).forEach(col => {
      initialCollapsedState[col] = false;
    });
    setCollapsedColumns(initialCollapsedState);
  }, [coc]);

  // Get input type based on valueType (copied from DomsTextField)
  ...
  // Get input props based on valueType (copied from DomsTextField)
  ...
  // Validate input based on valueType and validationRules (adapted from DomsTextField)
  ...
  // Note how rowValue/colValue parameters are swapped compared to vertical version
  const findCategoryOptionCombo = (
    rowValue: string, // Gender - now row
    colValue: string  // Age category - now column
  ...
  // Prepare payload for saving (adapted from DomsTextField)
  ...
  // Render the complete matrix table with horizontal orientation
        ...
        <Table size="small">
          <TableHead>
            <TableRow>
              ...
              {!isComponentCollapsed && matrix.columns.map((column, index) => (
                <TableCell 
                  key={index} 
                  align="center" 
                  sx={{ 
            ...
            {matrix.rows.map((row, rowIndex) => (
              ...
              >
                ...
                {!isComponentCollapsed && matrix.columns.map((column, colIndex) => {
                  // Row is gender and column is age category (flipped from vertical)
                  const coc = findCategoryOptionCombo(row, column);
                  if (!coc) {
                    console.warn(`Could not find category option combo for: ${row}, ${column}`);
...
export default HorizontalMatrixTextFieldBlock;